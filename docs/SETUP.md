...
3. エディタ左の「ファイル」で、`backend/` の各ファイルを同じ名前で作成し、中身を貼り付ける。
   ...
   - `Config.gs` / `Code.gs` / `Users.gs` / `SheetRepo.gs` / `DocRepo.gs` /
...
3. ログイン画面が出たら、**苗字を小文字のローマ字で入力**します（例: `tanaka`）。
   - 初めての苗字なら「新規登録しますか？」と確認が出ます。登録すると `Logs_<姓>` と
     `Sessions_<姓>` の2シートが自動で作られ、以降その人の記録はそこに溜まります。
   - 2人目以降も同じURLを開いて、それぞれの苗字でログインすれば使えます。
...
| 他の人の記録が見えてしまう | 苗字ログインは本人確認ではなくデータの切り分けです。URLとAPIキーを知っている人は、他人の苗字でログインできます |
